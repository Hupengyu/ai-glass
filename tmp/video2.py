...
class push_flush_1(object):
    ...
    def start_thread(self):
        print('进入start_1')
        thread1 = Thread(target=self.video())
        thread1.start()
        thread2 = Thread(target=self.push_frame())
        thread2.start()


class push_flush_2(object):3
        ...
        if len(rtsp_now) > 0:
            ...
            if rtsp_init == rtsp_now:  # 如果推流txt 有内容 如果内容无变化
                ...
                for URL in URLS:
                    ...
                    process_1 = ctx.Process(target=push_flush_1, args=(URL, frame_queue))
                    process_1.start()
