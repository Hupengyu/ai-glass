...
class Retinaface(object):
    ...
    def __init__(self, encoding=0, **kwargs):
        ...
        sql = "select * from face_feature_embedding"
        ...
        try:
            ...
            for result in results:
                print(result)
                name = result[0]
                face_embeding = result[1]

                dlist = face_embeding.strip(' ').split(',')
    ...
    def encode_face_dataset(self, image_paths, names):
        ...
        for index, path in enumerate(tqdm(image_paths)):
            ...
            with torch.no_grad():
                ...
                print(len(enconding_str))

                #数据插入数据库

                sql = 'INSERT INTO face_feature_embedding(name,embedding) VALUES(%s,%s)'
                try:

                    cursor.execute(sql,([names[index],enconding_str]))
                    print('Successful')
                    db.commit()
                except:
                    print('Failed')
